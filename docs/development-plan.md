# 分阶段开发计划

## 当前阶段：阶段 4 后台人工确认付款 + 自动发货

本阶段实现订单查询闭环、简单 token 保护后台、人工确认付款和文本库存发货。

本阶段明确不做：

- 不实现真实支付。
- 不实现用户注册。
- 不实现复杂后台权限系统。
- 不修改第一阶段页面设计风格。

## 阶段 1：项目初始化 + 前台基础页面

- 初始化 Next.js + TypeScript + App Router。
- 集成 Tailwind CSS 和 shadcn/ui 风格基础组件。
- 预留 Prisma + PostgreSQL 配置。
- 创建首页、产品中心、商品详情、订单查询、订单成功、文章、客服、政策和后台占位页面。
- 使用 mock 数据展示商品、文章和订单。

状态：已完成。

## 阶段 2：数据库模型 + seed 数据

- 设计 AdminUser、Category、Product、ProductVariant、InventoryItem、Order、OrderItem、DeliveryItem、PaymentRecord、ArticleCategory、Article、Setting 模型。
- 添加商品、库存、订单、支付、发货、文章状态枚举。
- 配置 Prisma Client、PostgreSQL datasource、索引和唯一约束。
- 创建基础 seed 数据：分类、商品、规格、文本库存、文章分类、文章和站点设置。
- 添加 `lib/db.ts` 作为 Prisma Client 单例入口。

状态：已完成。

## 阶段 3：商品读取 + 下单流程

- 从数据库读取商品和库存状态。
- 实现商品详情页真实规格、数量和联系方式表单。
- 创建订单记录，但仍不接真实支付。
- 创建人工支付说明页 `/order/[orderNo]/pay`。
- 订单保持 `pending_payment`，不扣减库存，不创建发货内容。

状态：已完成。

## 阶段 4：后台人工确认付款 + 自动发货

- 实现后台订单列表和订单详情。
- 管理员人工确认付款。
- 系统从文本库存中锁定并分配发货内容。
- 本阶段处理库存锁定、发货分配和订单完成状态。
- 实现订单号 + 联系方式查询订单结果。
- 已发货订单展示 DeliveryItem 发货内容，未发货订单不泄露库存内容。

状态：当前阶段。

## 阶段 5：订单查询增强和售后体验

- 优化订单查询体验、状态文案和异常提示。
- 补充售后联系入口和订单问题处理流程。
- 可加入基础操作记录和客服备注。

## 阶段 6：SEO 文章系统 + AI 生成草稿

- 实现文章管理、分类、slug、meta、canonical 和 FAQ。
- 接入 AI provider 生成 SEO 文章草稿。
- 管理员审核后发布。

## 阶段 7：正式支付网关预留/接入

- 抽象支付订单和回调接口。
- 接入正式支付网关。
- 处理支付成功回调和订单状态流转。

## 阶段 8：安全加固和部署

- 增加后台认证、权限、审计日志和基础风控。
- 配置生产环境变量、数据库、部署流水线和监控。
- 完成性能、安全和可用性检查。
